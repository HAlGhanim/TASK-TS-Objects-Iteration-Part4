...
function isCarColorMatching(car: Car, color: string): boolean {
  // write your code here...
  return car.color == color; // replace false with what you see is fit
}
...
function addCar(
  ...
  // write your code here...
  cars.push({ id, make, model, year, color });
...
function removeCarById(cars: Car[], id: number): Car[] {
  // write your code here...
  return cars.filter((car) => car.id !== id); // replace empty array with what you see is fit
}
...
function updateCarColor(
  ...
  if (!car) return "No Car Found";
  return { ...car, color: newColor }; // replace "No Car Found" with what you see is fit
}
